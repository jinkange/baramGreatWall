...
def search_images(screen, image_folder):
    global paused_until, found_flags, channel_num
    ...
    for filename in os.listdir(image_folder):
        ...
        if max_val >= 0.8:
            if not found_flags.get(filename, False):
                ...
                if(channel_num == ''):
                    channel_num = '매크로 시작 채널'
                
                data = {
                    "content": f"{filename} 몬스터 발견, 캐릭터명 : [{characterName}], 채널 : [{channel_num}]",
...
def search_loop():
    ...
    print("🔎 이미지 서치 루프 시작 (F7로 중지 가능)")
    while True:
        if searching:
            screen = capture_fullscreen()
            found = search_images(screen, "searchImages")
            # if not found:
